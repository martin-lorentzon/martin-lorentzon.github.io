...
- Repeated utility groups are extracted into semantic classes in `src/input.css` (`@layer components` + `@apply`, e.g. `page-section`, `section-title`, `copy-punchy`). Never `@apply` a daisyUI component class (`collapse`, `join-item`, `drawer`, `menu`, etc.) into one of these — daisyUI's behavior lives in parent/child selectors like `.collapse:has(input:checked) > .collapse-content`, and `@apply` copies only a class's own declarations, so the wrapper silently loses them (an FAQ item rendered permanently collapsed when tried). Keep daisyUI classes literal in the HTML; `@apply` plain Tailwind utilities only.
